...
def gaussian(prices, lat, lon, ignore=(0.001, 0.002)):
    num = 0
    dnm = 0
    c = 0
...
def absolute_rsrp_single(pricem, lat, lon):
    a = 0
    summ = []
    for _, plat, plon in pricem:
        #print(lat - plat, lon - plon)
        if distance_squared(plat, plon, lat, lon) < 0.016:
            summ.append(pricem[a][0])
        else:
            a += 1
    if a == len(pricem):
        return None
    ...
    for item in summ:
        medium += item
    medium = medium / len(summ)
    
    if medium >= -40:
       return 6000
    elif medium < -41 and medium >= -50:
       return 4500
    elif medium < -51 and medium >= -60:
       return 4000
    elif medium < -61 and medium >= -70:
       return 3800
    elif medium < -71 and medium >= -80:
       return 3400
    elif medium < -81 and medium >= -90:
       return 3000
    elif medium < -91 and medium >= -100:
       return 2800
    elif medium < -101 and medium >= -110:
       return 2200
    elif medium < -111 and medium >= -120:
       return 2100
    elif medium < -121 and medium >= -130:
       return 1900
    elif medium < -130:
       return 1500
...
def start(fname):
   ...
   # priced_points, slope, y_intercept = load_prices(fname)
    ...
    out_fname = "itog" + ".phantom." + str(MAX_X)
